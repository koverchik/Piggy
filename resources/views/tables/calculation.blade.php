<table class="table">
    ...
    @foreach($calculation as $item)
        <tr>
            <td>
                <a href="{{ Auth::user()->id == $item['user']->id ? route('user') : route('member', ['user' => $item['user']->id]) }}">
                @if($item['user']->avatar !== null)
                    <img width="30" height="30" src="{{ asset('storage/avatars/'. $item['user']->avatar) }}" class="m-1 cursor-pointer" alt="Avatar {{$item['user']->name}}"/>
                @else
