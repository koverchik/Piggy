...
const PaginationRows: React.FC = observer(() => {
  ...
  function createNumberPagination() {
    const resultPagination: any = store.Wallet.numberPagination.map(
      (item: number) => {
        return (
          <div
            key={'pagination' + item}
            className={
              store.Wallet.activePagination == item
  ...
  function clickPagination(e: any) {
    store.Wallet.activePagination !== e.target.textContent
      ? (store.Wallet.activePagination = e.target.textContent)
      : '';
  }
  return (
    <div className="wrapper-number-pagination">
      <img
        ...
        onClick={() => {
          store.Wallet.activePagination > 1
            ? (store.Wallet.activePagination =
                store.Wallet.activePagination - 1)
            : '';
        }}
